...
// Load defined components into redux for drag and drop
...
export const GenericComponent = (props) => {
  ...
  let customProp = isEmpty(pick([props.opts.type], store.getState().editor.customProps)) 
    ? {} 
    : pipe(
        pick([props.opts.type]),
        values, 
        head
      )(store.getState().editor.customProps)
  let parents = isEmpty(props.parentchain) 
    ? [props.parentkey] 
    : append(props.parentkey, props.parentchain);
  ...
  const emptyChildren = values(mapObjIndexed(slot => {
			return <EmptyChild key={slot} {...props} parentchain={parents} parentkey={slot} />;
		}, empties));
  ...
  if (!isNil(props.opts.children) && !isEmpty(props.opts.children)){
    const kids = pipe(
      mapObjIndexed((comp, index) => {
        return {
          sort: comp.sort, 
          compDef:  <FullChild key={index} {...props} {...customProp} parentchain={parents}>
    ...
    return (
      <Component {...props} {...props.opts.props}>
        {kids}
        {emptyChildren}
      </Component>
    );
    ...
    return (
      <FullChild key="drop-child" {...props} {...customProp} parentchain={parents}>
